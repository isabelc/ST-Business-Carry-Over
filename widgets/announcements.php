 ...
 */
 ...
 if ( ! class_exists( 'SmartestAnnouncements_Legacy' ) ) {
	...
	class SmartestAnnouncements_Legacy extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				...
				__( 'Smartest Announcements', 'st-business-carry-over-legacy' ),
				array( 'description' => __( 'Display the latest Announcements.', 'st-business-carry-over-legacy' ), )
			);
		}
		/**
		...
		public function widget( $args, $instance ) {
			$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Latest News', 'st-business-carry-over-legacy' ) : $instance['title'], $instance, $this->id_base );
			$number = isset( $instance['number'] ) ? $instance['number'] : 3;
			$see_all_label = ( ! empty( $instance['see_all_label'] ) ) ? $instance['see_all_label'] : __('All Announcements', 'st-business-carry-over-legacy');
			...
			*/
			...
			if ( $sbfnews->have_posts() ) { ?>
				...
				?> <p><?php printf(__( '%s', 'st-business-carry-over-legacy'), $li); ?></p>

		<?php } else {
					?>
					<p><?php _e('Coming soon.', 'st-business-carry-over-legacy'); ?></p>		
		<?php }
			wp_reset_postdata();
		...
		public function form( $instance ) {
			$title = isset( $instance[ 'title' ] ) ? $title = $instance[ 'title' ] : __( 'Latest News', 'st-business-carry-over-legacy' );
			$number = isset( $instance[ 'number' ] ) ? $instance[ 'number' ] : 3;
	...
	/* Default Widget Settings */
			...
			<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'How many recent announcements to show:', 'st-business-carry-over-legacy' ); ?></label> 
			...
			<p><label for="<?php echo $this->get_field_id( 'see_all_label' ); ?>"><?php _e( 'Replace the link label, "All Announcements", with your own text:', 'st-business-carry-over-legacy' ); ?></label>
